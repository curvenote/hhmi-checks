 ...
 *   npx tsx packages/checks-text-integrity/scripts/reconcile-check-run-columns.ts
 *   npx tsx packages/checks-proofig/scripts/reconcile-check-run-columns.ts
 */
import { getPrismaClient } from '@curvenote/scms-server';
import { isTextIntegrityRunFailed } from '../src/server/isRunFailed.server.js';
import {
  deriveTextIntegrityFailedAt,
  errorColumnPatch,
  healthyColumnPatch,
...
async function main() {
  ...
  console.log(
    JSON.stringify({ kind: TEXT_INTEGRITY_KIND, scanned, updated }, null, 2),
  );
}
