...
function phaseLabel(phase: TestError['phase']): string | undefined {
  ...
  if (phase === 'provider') return 'Turnitin (via relay)';
  return undefined;
}
...
export function TextIntegrityUpdateConfigurationForm({ credentials }: Props) {
  ...
  }, [statusOpen, credentials.apiBaseUrl, credentials.apiKey, credentials.keyName]);
  ...
  }, [configureOpen, credentials.apiBaseUrl, credentials.apiKey, credentials.keyName]);
