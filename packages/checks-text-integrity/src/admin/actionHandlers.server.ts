...
> {
  ...
  if (body.status === 'error') {
    const msg = typeof body.message === 'string' ? body.message : 'Turnitin / TCA configure failed';
    return {
      ok: false,
      error: {
        type: 'provider_error',
...
/**
 * POST …/status — service-level status; iThenticate returns raw TCA features-enabled JSON from the relay.
 */
