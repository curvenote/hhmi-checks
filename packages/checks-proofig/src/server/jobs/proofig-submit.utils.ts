...
export async function postToProofigStream(
  ...
  const pdfReadable = Readable.fromWeb(pdfResponse.body as unknown as NodeReadableStream<Uint8Array>);
  ...
  const response = await fetch(url, {
    ...
    // Stream body: duplex is required by Fetch when sending a streaming body (Node/undici).
    duplex: 'half',
